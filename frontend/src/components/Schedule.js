...
const Schedule = ({ user }) => {
  ...
  const fetchTasks = useCallback(async () => {
    try {
      const response = await axios.get(`${process.env.REACT_APP_API_URL}/api/schedule/${date}`, { withCredentials: true });
  ...
  const handleAddTask = async () => {
    ...
    try {
      const response = await axios.post(
        `${process.env.REACT_APP_API_URL}/api/schedule/${date}/${newTask.startTime}/${newTask.endTime}`, 
        newTask.description,
        {
          headers: {
  ...
  const handleSave = async () => {
    try {
      const response = await axios.put(
        `${process.env.REACT_APP_API_URL}/api/schedule/${selectedTask.id}`, 
